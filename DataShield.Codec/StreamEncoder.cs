...
///
/// Операции не изменяют состояние экземпляра (промежуточные буферы —
/// локальные, вайп выполняется по завершении), поэтому параллельные
/// вызовы <see cref="Encode(ReadOnlySpan{byte}, string)"/> безопасны.
/// </summary>
public sealed class StreamEncoder : StreamCodec
{
    ...
    /// <param name="sectorScheme">Схема хеширования секторов; по умолчанию Classic.</param>
    /// <param name="virtualIndexSectorLimit">
    /// Порог схемы VirtualIndex: максимум N+M томов (по умолчанию
    /// <see cref="VirtualIndexHasher.DefaultSectorLimit"/>). Файл с большим
    /// числом томов схемой VirtualIndex не кодируется (без даунгрейда).
    /// </param>
    ...
    /// <summary>
    /// M = max(1, round(N · eccPercent / 100)) при eccPercent ≥ 1, иначе 0.
    /// Округление — к ближайшему (полутона вверх).
    /// </summary>
    public static int ComputeEccCount(int dataCount, int eccPercent)
    {
        if (eccPercent <= 0 || dataCount <= 0) return 0;
        var m = (int)(((long)dataCount * eccPercent + 50) / 100);
        return Math.Max(1, m);
    }
    ...
    /// <summary>
    /// Количество копий заголовка: max(3, round(totalCount · headerPercent / 100)).
    /// Округление — к ближайшему (полутона вверх), минимум 3 копии.
    /// </summary>
    public static int ComputeHeaderCount(int totalCount, int headerPercent)
    {
        ...
        var pct = (totalCount * headerPercent + 50) / 100;
        ...
        IProgress<CodecProgress>? progress, CancellationToken ct)
    {
        ...
        if (fileSize > PacketFormat.MaxFileSizeField)
            throw new InvalidOperationException(
                CodecErrors.FileSizeExceedsMax(fileSize, PacketFormat.MaxFileSizeField));
        ...
        if (totalCount > PacketFormat.MaxDataVolumes)
            throw new InvalidOperationException(
                CodecErrors.TotalVolumesExceedMax(totalCount, PacketFormat.MaxDataVolumes));

        if (SectorScheme == SectorScheme.VirtualIndex && totalCount > VirtualIndexSectorLimit)
            throw new InvalidOperationException(
                CodecErrors.VirtualIndexVolumeLimit(totalCount, VirtualIndexSectorLimit));
        ...
        if (eccCount > 0)
        {
            ...
            var rsResult = Rs.Encode(dataPayloads, eccCount, eccProgress, ct);
        ...
        for (var i = 0; i < totalCount; i++)
        {
            ...
            dataSectors[i] = SectorScheme == SectorScheme.VirtualIndex
                ? BuildVirtualIndexSector(i, allPayloads[i], headerHash)
                : BuildDataSector(i, allPayloads[i], headerHash);
        }
        ...
        IProgress<CodecProgress>? progress, CancellationToken ct)
    {
        ...
        // Размерности и SHA-256 пересчитываются здесь, а не возвращаются из
        // Encode, чтобы ядро кодирования оставалось свободным от статистики
        var fileSize = (uint)content.Length;
        var sha256 = Sha256Compact.HashData(content);
        var dataCount = Math.Max(1,
            (int)((fileSize + (uint)PacketFormat.PayloadSize - 1) / PacketFormat.PayloadSize));
        ...
        // Копии заголовка = остаток потока после data- и ECC-томов
    ...
    /// <summary>
    /// Сектор данных схемы VirtualIndex: payload(64) + Trunc11(11).
    /// Хеш = Trunc11(SHA-256(headerHash ‖ payload ‖ seqNum LE)); номер
    /// сектора в пакет не записывается (виртуальный индекс).
    /// </summary>
    internal static byte[] BuildVirtualIndexSector(
        int sectorNum, byte[] payload, byte[] headerHash)
    {
        var sector = new byte[PacketFormat.PacketSize];
        VirtualIndexHasher.BuildPacketInto(payload, sectorNum, headerHash, sector);
        return sector;
    }
}
...
/// <summary>
/// Тесты ReceptionSlot — слота одного файла в накопителе приёма:
/// накопление и ранжирование версий секторов, карты валидности и коллизий,
/// сборка файла (прямая; полный перебор комбинаций; эвристическая синхронная
/// прокрутка; подбор подмножества томов для RS-восстановления), бюджет
/// времени, отмена, прогресс, а также сериализация HeaderContent.
/// </summary>
public sealed class ReceptionSlotTests
{
    ...
    /// <summary>
    /// Точные копии одного payload схлопываются в единственную версию:
    /// ConfirmationCount растёт с числом приёмов, копии считаются,
    /// коллизией это не считается.
    /// </summary>
    [Theory]
    [InlineData(1)]
    [InlineData(2)]
    ...
    /// <summary>
    /// Разные payload одного сектора порождают разные версии (по одному
    /// подтверждению у каждой); при равных счётчиках сохраняется порядок
    /// поступления, сектор фиксируется как коллизия.
    /// </summary>
    [Theory]
    [InlineData(2)]
    [InlineData(3)]
    ...
    /// <summary>
    /// Версии ранжируются по ConfirmationCount по убыванию; при равных
    /// счётчиках ожидаемый порядок — первоначальный порядок поступления.
    /// </summary>
    [Theory]
    [InlineData(3, 2, 1)]
    [InlineData(1, 2, 3)]
    ...
    /// <summary>
    /// Продвижение наверх: версия, набравшая больше подтверждений,
    /// вытесняет первую с вершины списка версий.
    /// </summary>
    [Theory]
    [InlineData(2)]
    [InlineData(3)]
    ...
    /// <summary>
    /// Неверный номер сектора или длина payload ≠ 64 байта — AddSector
    /// возвращает false, статистика слота не меняется.
    /// </summary>
    [Theory]
    [InlineData(-1, 64)]
    [InlineData(2, 64)]
    ...
    /// <summary>
    /// Карта валидности: несколько версий занимают один логический слот
    /// (один true); текстовая карта помечает коллизию «▓», единственную
    /// версию «█»; Coverage — доля принятых логических секторов.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Прямая сборка полного набора секторов: усечение паддинга до FileSize
    /// и сверка SHA-256. Длины — вокруг границ секторов (кратность 64 ± 1).
    /// </summary>
    [Theory]
    [InlineData(1)]
    [InlineData(2)]
    ...
    /// <summary>
    /// Отсутствует один data-сектор — прямой сборкой файл не собирается
    /// (null без обращения к ECC).
    /// </summary>
    [Theory]
    [InlineData(2, 1)]
    [InlineData(3, 1)]
    ...
    /// <summary>
    /// Приоритет по подтверждениям: верная версия, пришедшая позже, но
    /// набравшая больше подтверждений, идёт первой и сборка succeeds.
    /// </summary>
    [Theory]
    [InlineData(2)]
    [InlineData(3)]
    ...
    /// <summary>
    /// Полный перебор (одометр по комбинациям): верные версии добавлены
    /// последними — худший случай, комбинация всё равно находится.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// Перебор комбинирует неоднозначные секторы с однозначными:
    /// варьируются только секторы с коллизиями, остальные фиксированы.
    /// </summary>
    [Theory]
    [InlineData(1)]
    [InlineData(2)]
    ...
    /// <summary>
    /// В переборе участвуют только варианты с максимальным счётчиком:
    /// верная версия с меньшим числом подтверждений не проверяется —
    /// честный отказ (null).
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Равные счётчики «второго эшелона» не создают равновероятности
    /// с лидером: в перебор идёт только наиболее подтверждённый вариант.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Эвристическая синхронная прокрутка: верные версии стоят на одном
    /// индексе во всех списках (синхронная комбинация) и находятся
    /// за несколько прокруток; после успеха верные версии — первые.
    /// </summary>
    [Theory]
    [InlineData(1, 2, 1)]
    [InlineData(2, 2, 1)]
    ...
    /// <summary>
    /// Прокрутка списков разной длины: состояния повторяются через НОК
    /// длин (2 и 3 → 6); проверяются разные фазы синхронизации.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// Лимит попыток: исходное состояние считается первой попыткой,
    /// при maxAttempts = 4 индекс 4 уже не проверяется — отказ.
    /// </summary>
    [Theory]
    [InlineData(5, 4, 4)]
    [InlineData(6, 5, 5)]
    ...
    /// <summary>
    /// После неудачного поиска порядок версий остаётся прокрученным
    /// (wrong1, wrong2, wrong0) — состояние сохраняется между вызовами.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Новое подтверждение после прокрутки возвращает счётчику главенство:
    /// wrong0 с 2 подтверждениями поднимается на первое место.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Бюджет времени 1 мс: проверяется между попытками, поэтому сборка
    /// обязана отказаться быстро, а не исчерпывать лимиты попыток.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Документированная неполнота прокрутки: при длинах списков (2,2,2)
    /// НОК = 2, достижимы только «диагональные» комбинации; смешанная
    /// (0,1,0) недостижима — честный отказ.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Контроль к предыдущему тесту: полный перебор посещает все
    /// комбинации и находит смешанную (0,1,0), недостижимую прокруткой.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Коллизия версий на ECC-томе: перебор обязан варьировать именно
    /// ECC-версию — подделка портит RS-восстановление пропавшего data-тома.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Контроль: единственная (поддельная) версия ECC без коллизии —
    /// точек ветвления нет, RS восстанавливает мусор, SHA-256 не сходится.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Стадия 3 (подбор подмножества): тихо испорченный data-том принят
    /// одной версией — исключение его из RS-системы восстанавливает файл.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Тихо испорченный ECC-том плюс дыры в data: исключение испорченной
    /// чётности переключает RS на валидные уравнения.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Комбинация механизмов: коллизия на data-томе 0 (в базу исключений)
    /// плюс тихо испорченный том 2; перебор версий комбинируется
    /// с исключением повреждённого тома.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// База исключений (оба коллизионных тома) не влезает в бюджет ECC —
    /// стадия 3 пропускается, результат — честный отказ.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Два тихо испорченных data-тома при M=1: пары исключений неосуществимы
    /// (2 > 1), одиночные попытки оставляют второй повреждённый том в доверии —
    /// все маски исчерпаны, честный отказ.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Бюджет стадии 3 в 1 тик: сборка отказывается быстро, без зависания
    /// на переборе масок.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Прогресс стадии 3: до успеха все отчёты < 100%, финал — 100%
    /// с фазой AssemblyFinished; в серии присутствует фаза VolumeSubsetSearch.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Отмена из колбэка прогресса при первом же отчёте стадии 3 —
    /// OperationCanceledException до самих попыток.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Троттлинг прогресса перебора версий: до фактической сборки проценты
    /// строго меньше 100.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Снимок GetSectorVersions содержит копии: позднейшие подтверждения
    /// меняют слот, но не ранее снятый снимок.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Заранее отменённый токен — немедленная OperationCanceledException
    /// до любой работы.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Нулевые или отрицательные параметры поиска (число комбинаций,
    /// попыток, бюджет) — ArgumentOutOfRangeException из конструктора.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// Число data-томов выводится из FileSize: ceil(size/64);
    /// длины — вокруг границ секторов (кратность 64 ± 1).
    /// </summary>
    [Theory]
    [InlineData(1, 1)]
    [InlineData(2, 1)]
    ...
    /// <summary>
    /// TotalVolumeCount = DataVolumeCount + EccCount при различных
    /// сочетаниях размера файла и избыточности.
    /// </summary>
    [Theory]
    [InlineData(1, 0)]
    [InlineData(1, 1)]
    ...
    /// <summary>
    /// Сериализация заголовка: ToBytes → ReadFrom сохраняет все поля,
    /// размер сериализации — ровно HeaderContentSize байт.
    /// </summary>
    [Theory]
    [InlineData(1, 0)]
    [InlineData(64, 1)]
    ...
    /// <summary>
    /// Слот с полностью корректным заголовком для файла (H5 вычислен честно).
    /// </summary>
    ...
    /// <summary>Заголовок «test.dat» с SHA-256 файла и заданным eccCount.</summary>
    ...
    /// <summary>
    /// Нарезка файла на секторы по PayloadSize байт (последний — с паддингом).
    /// </summary>
    ...
    /// <summary>
    /// Добавить variantCount версий сектора: все по одному подтверждению,
    /// верная — на позиции correctVariantIndex, остальные — порченные
    /// солёные копии (воспроизводимо зависят от номера сектора и варианта).
    /// </summary>
    ...
    /// <summary>
    /// Гарантированно отличная от оригинала копия payload: два байта
    /// инвертируются по маскам, выведенным из salt.
    /// </summary>
    ...
    /// <summary>Отличимый от других вариант payload фиксированной длины.</summary>
    ...
    /// <summary>Склеить массивы байтов в один (в порядке следования).</summary>
    ...
    /// <summary>k псевдослучайных payload заданной длины из общего Random.</summary>
    ...
    /// <summary>Сравнение payload с ReadOnlyMemory через SequenceEqual.</summary>

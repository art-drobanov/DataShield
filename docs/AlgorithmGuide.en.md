# DataShield Algorithms — An Engineering Walkthrough

An engineering walkthrough of the DataShield codec's algorithmic core: the packet format, encoding and decoding logic, redundancy math, sector accumulation, targeted rebinding, and file assembly with version-collision resolution. No formalism for its own sake — shorter notation, more "why it is built this way" and "how it behaves on a real stream". This document covers algorithms and data structures only; the public API and solution layout are described in `DeveloperGuide.en.md`. Each task is additionally covered by its own document: encoding — `EncoderGuide.en.md`, decoding — `DecoderGuide.en.md`, assembly and collisions — `AssemblyGuide.en.md`, the sector-scheme collision bench — `CollisionBench.en.md`. The optional virtual-index sector scheme (VirtualIndex, up to 512 volumes) is described in sections 2.4 and 7.4.

## 1. Purpose and Principles

DataShield is a forward error correction (FEC) codec for transferring small files over unreliable one-way channels: text pastes, messengers, e-mail bodies, raw byte streams. A file becomes a stream of small self-contained packets; the decoder rebuilds the file from an arbitrarily damaged, reordered, duplicated, and multi-file stream. The channel is assumed to offer neither retransmission nor ordering — the architecture targets the worst case from the start.
...
The core design idea: **no byte of the input stream is ever treated as garbage until it fails a cryptographically strong checksum**. Everything else follows: lossless accumulation, deferred authenticity decisions, exhaustive re-scans.

## 2. Packet Format

Every packet has the same size — 75 bytes (`PacketFormat.PacketSize`). The size is not arbitrary: in text mode a packet is Base64-encoded, and 75 · 4/3 = exactly **100 characters with no padding** — one line = one packet, no tails, no alignment. A fixed size also means the scanner needs exactly one window per format.
...
The H1 field stores not the full name but a packed one (`FileNameCodec.Pack`): the name is split at the **first** dot; the extension (including compound ones like ".tar.gz") is kept intact while the base is truncated with a trailing `~` marker (`documents.tar.gz` → `docume~.tar.gz`). If less than 2 bytes remain for the base, the name is unrepresentable and encoding is rejected: silently mangling a name is worse than refusing. The full name is recovered via the SHA-256 (H3) by the file's owner.
...
- **The header hash (H5) is autonomous.** Trunc24(SHA-256(H1–H4)) is verified with no context. This lets the scanner recognize a header in a noisy stream "from scratch": the false positive probability is ~2⁻¹⁹² — at real stream volumes, false headers simply do not happen.
...
Why two levels: the header must be recognizable in isolation (it is the file's "entry door" into the stream), while a sector is verifiable only in its file's context. A single-level scheme would either fail to find headers or let sectors drift between files.
...
2. A sector arriving **before** its header is not recognized on the first pass (H5 is unknown yet). It is not lost: it is picked up later by a targeted rebinding of retained data (section 8).
...
### 2.4. The optional sector scheme: VirtualIndex

Besides the classic layout there is a second sector layout — the **virtual-index scheme** (B), enabled explicitly by the `sectorScheme: SectorScheme.VirtualIndex` parameter on the encoder and decoder (by default both sides operate in `Classic`):

| Field | Offset | Size | Contents                                        |
| ----- | ------ | ---- | ----------------------------------------------- |
| B1    | 0      | 64   | Payload                                         |
| B2    | 64     | 11   | Trunc11(SHA-256(H5 ‖ payload ‖ idxLE)) — 88 bits |

The sector number is **not stored** in the packet: it exists only inside the hash input (the same 90 bytes as A, but ordered `H5 ‖ payload ‖ idxLE`) and is recovered by the receiver through brute force. The freed 2 bytes go to the hash: truncation 9 → 11 bytes. Strength grows by exactly the bits moved from the explicit field into the hash, minus the scan cost.

- Applicability: N+M ≤ `VirtualIndexHasher.DefaultSectorLimit = 512` — both the default value of the `virtualIndexSectorLimit` parameter and the hard bound of its range. At N+M = 512 the effective strength is 79 bits versus 72 for A (scheme B loses log2(N+M) of its 88 bits to the index scan).
- The scheme is not signaled in the stream: it is a per-side setting. Reception is hybrid — the classic check first, then the virtual-index scan — so one decoder reads files of both schemes simultaneously (section 7.4).
- Reception cost: up to N+M hashes per packet; the arrival cursor makes an ordered stream O(1) (section 7.2). Swapping valid sectors is not detected by the scheme — and does not need to be: the arbiters remain the file's SHA-256 (H3) and RS recovery.
- Strength and cost measurements, and the rationale for the 512 ceiling — the collision bench (`CollisionBench.en.md`).
...
6. **Header:** serialize H1–H4 (51 bytes) → Trunc24(SHA-256) → header packet. The H5 value is remembered — from here on it is the seed for every sector.
7. **Sectors:** for each volume i ∈ [0, N+M) a packet `seqNum(2) + payload(64) + hash(9)`, where the hash is computed with seed = H5; under `SectorScheme.VirtualIndex` a `payload(64) + Trunc11(...)` packet is built instead (`BuildVirtualIndexSector`, section 2.4) — after checking N+M ≤ 512 up front (a violation refuses the job before any work, no silent downgrade to Classic).
...
9. **Wipe** of intermediate buffers (data/ECC payloads, serialized header) — only the returned packets remain in memory.
...
Why an erasure code rather than error correction: the integrity of every received volume is already guaranteed by truncated SHA-256 (missing a forgery with probability ~2⁻⁷²). The codec only needs to restore **gaps**, not fix distortions — radically simpler and faster. A classic division of labor: the hash answers "corrupted or not", the code answers "restore what is missing".
...
The post-success jump by the packet length is a direct-pass optimization, not a correctness property. It can skip the start of another valid packet overlapping the one just accepted (this actually happens on damage: a fragment of one volume together with the first characters of a neighboring line accidentally decodes into a valid packet). Coverage completeness does not depend on the jumps: the rebinding re-scan (section 8) checks the retained data at **every** position and never jumps at all.
...
Thus desynchronization, noise, and line breaks between packets are handled automatically: the window "feels out" the next packet boundary by shifting byte by byte. No offsets need to be known in advance — this is what makes the format self-synchronizing after arbitrary corruption.
...
The direct pass costs O(stream length) window invocations in the worst case; on a dense stream (packets back to back) — O(length/75) thanks to the jumps. Packets skipped by an overlapping jump are closed by the exhaustive re-scan (section 8).

The scanner **retains the entire processed stream** (input volumes are small — up to ~4 MB, an acceptable price) — this is what buys targeted rebinding: data unrecognized on the first pass is never lost.
...
2. **Otherwise** → try to accept as a sector for **every** known slot (`AcceptSectorForSlot`). The check is hybrid: the classic one first (the number is in `[0, N+M)` and the hash seeded with that slot's H5 matches), then — only if the slot operates in `SectorScheme.VirtualIndex` with N+M ≤ the limit — the virtual-index scan from the arrival cursor (section 7.4). One packet can theoretically be a valid sector of several slots — it is accepted into all of them; conflict resolution is deferred to assembly.

A recognized packet is emitted down the pipeline (`EmitPacket` — indivisible portions). New slots are collected into a list, and **after leaving the lock** `HeaderAccepted(header, headerHash)` is raised for each — a signal for upper modules to perform a targeted rebind (section 8). Raising events outside the lock is deliberate: the event handler launches a heavy rescan, and holding the lock across it is a sure path to a deadlock.
...
Why versions instead of "last one wins": a channel may duplicate and delay packets. If two different copies of the same number pass the hash check (an extremely unlikely but finite event — or a deliberate forgery), the accumulator keeps **both** with their counters. Reception is lossless: the decision of which version is correct is postponed until assembly, where an independent arbiter exists — the SHA-256 of the whole file. Discarding at reception would be an irreversible loss.

For the VirtualIndex scheme the slot additionally holds the **arrival cursor** (`VirtualIndexCursor`, `AdvanceVirtualIndexCursor`): the index scan starts at the expected next index, so an ordered stream costs one hash per packet. On a match the cursor advances; the sector number is recovered before entering the slot, so versions and assembly work exactly as in the classic scheme.
...
### 7.4. Sector reception in the VirtualIndex scheme

For a B sector the number is unknown, and the classic check does not apply. Reception (`VirtualIndexHasher.TryVerifyPacket`) scans the indices from the arrival cursor around the ring (cursor → K−1 → 0 → cursor−1) and returns the first match:

- the scan is two-phase: the first 64 steps sequential (the O(1) path of an ordered stream does not pay for parallelism); with ≥ 256 candidates remaining — parallel blocks of 128 steps (`Parallel.For`); the match with the lowest step number wins, which is identical to the sequential scan (determinism beats peak speed);
- with a uniformly distributed true index a valid sector matches at step (K+1)/2 on average; a foreign packet costs all K hashes (worst case K = 512 ≈ 0.24 ms per stream, divided by roughly the core count);
- ambiguity (two indices with matching hashes) is resolved by the first in scan order; the probability is (K−1)·2⁻⁸⁸ ≈ 2·10⁻²⁴ at K = 512 — beyond any practical scenario.

## 8. Targeted Rebinding: a Sector Before Its Header

The problem: a sector that arrives before the first header of its file is not recognized on the first pass — H5 is unknown, and the hash check is impossible (an autonomous header-style hash over D1–D2 almost never matches D3). The scanner shifts by 1 byte and moves on. On a real multi-file stream with interleaved packets this is the norm, not an edge case.
...
3. The sector hash seeded with the **new** header's H5 is verified (for VirtualIndex slots — the same hybrid pair: the classic check, then the virtual-index scan from a zero cursor).
...
Assembly (`ReceptionSlot.TryAssemble`) is the codec's most involved algorithm. The authenticity arbiter is the whole-file SHA-256 from H3: assembly is either provably correct or honestly refuses. What follows is a working overview; the detailed treatment (structure invariants, exact search-space definitions, the mathematics of the odometer and rotation, worked examples, guarantee boundaries) is in `AssemblyGuide.en.md`.
...
A separate console bench, `DataShield.CollisionBench`, compares the collision strength and reception cost of the two sector schemes (classic and VirtualIndex) — methodology, model, and results are in `CollisionBench.en.md`.
...
| Sector scheme            | Classic (2+64+9); optionally VirtualIndex (64+11, N+M ≤ 512, 79 bits) |

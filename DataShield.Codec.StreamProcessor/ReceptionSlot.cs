...
{
    // Сериализованный заголовок (для побайтового сравнения при привязке копий)
    ...
    // Настройки поиска версий (лимиты перебора, эвристики, стадии 3)
    ...
    /// <summary>
    /// Курсор прихода схемы VirtualIndex: ожидаемый следующий индекс сектора.
    /// Продвигается при каждом принятом секторе этой схемы. Для
    /// последовательных потоков перебор индексов на приёме сокращается до
    /// O(1); для перемешанных порядок обхода остаётся полным.
    /// </summary>
    internal int VirtualIndexCursor { get; private set; }

    /// <summary>
    /// Продвинуть курсор прихода VirtualIndex на индекс, следующий за
    /// принятым (циклически).
    /// </summary>
    internal void AdvanceVirtualIndexCursor(int acceptedIndex) =>
        VirtualIndexCursor = TotalVolumeCount > 0
            ? (acceptedIndex + 1) % TotalVolumeCount
            : 0;
    ...
    /// <summary>
    /// Флаги коллизий для карты валидности: true для номеров секторов,
    /// у которых принято более одной различающейся версии payload.
    /// null, если коллизий нет. Длина = TotalVolumeCount.
    /// </summary>
    public bool[]? BuildCollisionFlags()
    {
        var collisions = BuildCollisionMap();
        if (collisions.Count == 0) return null;

        var flags = new bool[TotalVolumeCount];

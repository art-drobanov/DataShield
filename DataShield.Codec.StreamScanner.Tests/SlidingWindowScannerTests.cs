...
/// <summary>
/// Тесты SlidingWindowScanner: сканера потока с окном фиксированной длины.
///
/// Обработчик окна возвращает шаг продвижения (совпадение — прыжок на длину
/// окна, иначе — сдвиг на 1). Проверяются: распознавание и продвижение,
/// окна на границах кусков, хвосты короче окна, прогресс Consumed/Retained,
/// адресная перепривязка RequestRescan (отложенно во время активного прохода,
/// с ограничением границей выдачи) и удержание данных между источниками.
/// </summary>
public sealed class SlidingWindowScannerTests
{
    ...
    /// <summary>
    /// Прогнать вход через сканер кусками по chunkSize и собрать выдачи.
    /// </summary>
    ...
    /// <summary>ASCII-байты строки.</summary>
    private static byte[] Bytes(string s) => s.Select(c => (byte)c).ToArray();

    // ── Прямой проход ───────────────────────────────────────────────────────

    /// <summary>
    /// Каждый распознанный образец продвигает окно на свою длину —
    /// выход содержит ровно байты всех совпадений.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Нераспознанное окно сдвигается по одному байту до следующего
    /// совпадения.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Окно, разрезанное границей куска источника, собирается из буфера
    /// и распознаётся.
    /// </summary>
    [Fact]
    ...
    /// <summary>Вход короче окна — ни одного сканирования.</summary>
    [Fact]
    ...
    /// <summary>Хвост короче окна не сканируется и не выдаётся.</summary>
    [Fact]
    ...
    /// <summary>Нулевое окно и null-обработчик — исключения конструктора.</summary>
    [Fact]
    ...
    /// <summary>
    /// Событие ConsumedAdvanced отчитывает продвижение; после Complete
    /// потреблено и удержано одинаково (хвостов не осталось).
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Перепривязка простаивающего сканера: повторный проход по всем
    /// удержанным данным распознаёт окна, пропущенные основным проходом.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Запрос перепривязки во время активного прохода откладывается
    /// и ограничивается границей уже выданных данных: то, что выдано,
    /// перепривязывается, последующие байты — нет.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Прямой проход «прыгает» по совпадениям и может перепрыгнуть начало
    /// перекрывающегося окна; перепривязка проверяет каждую позицию
    /// и находит пропущенное.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Удержанные данные переживают Detach/Attach: накопленный от первого
    /// источника кусок участвует в перепривязке, второй источник
    /// продолжает прямой проход.
    /// </summary>
    [Fact]

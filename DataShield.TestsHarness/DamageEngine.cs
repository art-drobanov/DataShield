...
public static class DamageEngine
{
    ...
    // перебирает одиночные исключения всех одноверсионных томов — до N
    // RS-декодов по O(N) каждый (стоимость склина квадратична по объёму).
    // Подделка занимает случайное место в порядке подозрительности (гигантская
    // связка равноподтверждённых томов), поэтому ожидаемое время находки —
    // половина склина. Лимит откалиброван по факту: при N+M = 3945 попытка
    // стоит ~12 мс, находка на ~2400-й — 29 с, впритык к бюджету стадии
    // (30 с); под нагрузкой бюджет исчерпывается и корректный отказ ломает
    // ожидание нормальной итерации. Порог 1024 держит худший случай (полный
    // склин) в единицах секунд с запасом на порядок — и покрывает ~88%
    // лог-равномерных размеров файлов стендов.

    /// <summary>Максимальное суммарное число томов файла (N+M) для тихой порчи.</summary>
    private const int SubsetStageVolumeLimit = 1024;
    ...
    /// <param name="sectorScheme">Схема хеширования секторов файла.</param>
        ...
        SectorScheme sectorScheme = SectorScheme.Classic)
    {
        ...
        var damaged = DamagePackets(
            ...
            overkillExtra, collisionKill, sectorScheme, ref mask, ref lost, ref ops,
            out var budget);

        // Рендер повреждённого пакета в куски заданного формата
    ...
    /// <param name="schemes">Схема хеширования секторов каждого файла
    /// (null — Classic для всех).</param>
    public static DamageResult ApplyMultiFile(
        ...
        IReadOnlyList<SectorScheme>? schemes = null)
    {
        ...
        for (var f = 0; f < files.Count; f++)
        {
            ...
            var scheme = schemes is null || f >= schemes.Count
                ? SectorScheme.Classic
                : schemes[f];

            var damaged = DamagePackets(
                packets, stats.EccCount, stats.DataCount + stats.EccCount, h5, rng,
                overkill, collisionKill: false, scheme, ref mask, ref lost, ref ops,
                out _);
    ...
    /// <summary>
    /// Многофайловый поток без повреждений: FEC-потоки файлов следуют
    /// строго последовательно (как консольный mlt-сценарий), пакеты в
    /// исходном порядке, единственный кусок в случайно выбранном формате
    /// (Binary или Base64). Проверяет переход декодера между файлами в
    /// чистом последовательном приёме — без перестановок и мусора.
    /// </summary>
    /// <param name="files">Пакеты и статистика каждого файла.</param>
    /// <param name="rng">Генератор случайных чисел.</param>
    public static DamageResult ConcatMultiFile(
        IReadOnlyList<(IReadOnlyList<byte[]> Packets, EncodeStats Stats)> files,
        Random rng)
    {
        var mask = DamageBits.MultiFile;
        var ops = 0;

        if (rng.Next(2) == 0)
        {
            var stream = new List<byte>();
            foreach (var (packets, _) in files)
                foreach (var packet in packets)
                    stream.AddRange(packet);

            ops++;
            return new DamageResult(
                mask, LostSectors: 0, ops,
                new[] { stream.ToArray() },
        ...
        var lines = new List<string>();
        foreach (var (packets, _) in files)
            lines.AddRange(packets.Select(p => Convert.ToBase64String(p)));
    ...
    private static List<byte[]> DamagePackets(
        ...
        Random rng, int overkillExtra, bool collisionKill, SectorScheme scheme,
        ref uint mask, ref int lost, ref int ops, out int budget)
    {
        ...
        if (budget > 0)
        {
            var corrupted = PacketDamage.CorruptSectors(
                list, Math.Min(budget, 1 + rng.Next(3)), h5, rng, scheme, totalVolumes);
            if (corrupted > 0)
            {
                budget -= corrupted;
                lost += corrupted;
        ...
        if (budget > 0)
        {
            var removed = PacketDamage.RemoveSectors(
                list, Math.Min(budget, 1 + rng.Next(2)), h5, rng, scheme, totalVolumes);
            if (removed > 0)
            {
                budget -= removed;
                lost += removed;
        ...
        if (budget > 0 && overkillExtra == 0 && !collisionKill &&
            ...
            Roll(rng, 0.35))
        {
            if (PacketDamage.CorruptSilently(list, h5, rng, scheme, totalVolumes))
            {
                budget--;
                lost++;
        ...
        if (overkillExtra > 0)
        {
            var extra = PacketDamage.RemoveSectors(list, overkillExtra, h5, rng, scheme, totalVolumes);
            if (extra == 0)
                extra = PacketDamage.CorruptSectors(list, overkillExtra, h5, rng, scheme, totalVolumes);
            lost += extra;
        ...
        if (collisionKill)
        {
            if (PacketDamage.InjectCollisionKill(list, h5, rng, scheme, totalVolumes))
            {
        ...
        else if (overkillExtra == 0 && Roll(rng, 0.45))
        {
            var injected = Roll(rng, 0.4)
                ? PacketDamage.InjectCollisionTie(list, h5, rng, scheme, totalVolumes)
                : PacketDamage.InjectCollisionPromote(list, h5, rng, scheme, totalVolumes);
            if (injected)
            {
    ...
    /// <summary>
    /// Рендер в текстовые куски: мусор, фрагменты, декорация Base64,
    /// иногда приём в два прохода или смешанные txt+bin куски.
    /// </summary>
    ...
    /// <summary>
    /// Рендер в бинарный поток: планирование шумовых вставок (рассинхронизация,
    /// текстовый разрыв, фрагмент, шум по краям) на выровненной сетке пакетов,
    /// затем применение и, возможно, разрез на два прохода.
    /// </summary>
    ...
    /// <summary>True с вероятностью probability.</summary>
    private static bool Roll(Random rng, double probability) =>
        rng.NextDouble() < probability;

    /// <summary>Строки → UTF-8 байты куска с переводом строки в конце.</summary>

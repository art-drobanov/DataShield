...
/// <summary>
/// Сеточный бенчмарк стоимости RS-декодирования: фиксированные N × доли
/// ECC × число стёртых data-томов E; на выходе — миллисекунды на попытку
/// (Init + 32×Process + SHA-256) и оценка числа попыток в 30-секундном
/// бюджете стадии подбора подмножества томов.
/// </summary>
internal static class Bench
{
    ...
    public static void Run(bool csv)
    {
        ...
        else
        {
            ...
            Console.WriteLine(" " + Program.VersionLine);
            Console.WriteLine();
            Console.WriteLine(
    ...
    /// <summary>M = max(1, round(N · percent / 100)) — как в StreamEncoder.</summary>
    private static int ComputeEccCount(int dataCount, int percent) =>
        Math.Max(1, (int)(((long)dataCount * percent + 50) / 100));

    // ── Замер одной ячейки ──────────────────────────────────────────────────

    /// <summary>
    /// Замер ячейки (N, M, E): три стадии попытки — Init (матрица
    /// + инверсия), 32 × Process (восстановление символов) и SHA-256
    /// буфера N·64. Возвращает Cell с длительностями стадий.
    /// </summary>
    ...
    /// <summary>Строка ячейки: CSV (N;M;E;…) или колонка таблицы.</summary>
